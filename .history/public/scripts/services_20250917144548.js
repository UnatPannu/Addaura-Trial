...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Your existing scroll spy on window
  ...
  // Insert scroll pause/hold here using wheel event on window
  ...
  window.addEventListener("wheel", (e) => {
    ...
    let nextIndex = direction > 0 ? currentSectionIndex + 1 : currentSectionIndex - 1;
    if (nextIndex < 0 || nextIndex >= sections.length) return;

    let nextSectionTop = sections[nextIndex].offsetTop;
    let distanceToNext = nextSectionTop - currentScroll;
  ...
  // Your existing label click handler ...
