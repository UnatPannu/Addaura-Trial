...
document.addEventListener('DOMContentLoaded', () => {
  const firstSection = document.querySelector('.intro-card-wrapper:nth-child(1)');
  const navbar = document.querySelector('nav.navbar.light-navbar, nav.navbar');
  ...
  firstSection.addEventListener('click', () => {
    // Toggle expanded class on firstSection
    firstSection.classList.toggle('expanded');

    // Also toggle intro section expanded state as in your design
    ...
    // Sync navbar class based on whether first card is expanded
    if (firstSection.classList.contains('expanded')) {
      // Change navbar to dark version - replace light-navbar class with navbar
      navbar.classList.remove('light-navbar');
      navbar.classList.add('navbar');
    } else {
      // Change navbar back to light version
