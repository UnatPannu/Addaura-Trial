...
document.addEventListener('DOMContentLoaded', () => {
  const container = document.querySelector('.for-teams-and-talent .sections-container');
  if (!container) return;
  ...
  sections.forEach(s => s.setAttribute('aria-expanded', s.classList.contains('open') ? 'true' : 'false'));
  ...
  sections.forEach(section => {
    // choose a sensible click target: prefer a header inside .section if present
    const clickTarget = section.querySelector('.mobile-heading') || section.querySelector('h4') || section;

    clickTarget.addEventListener('click', (e) => {
      // don't toggle when user clicks a real interactive element inside header
      if (e.target.closest('a, button, input, textarea')) return;

      const wasOpen = section.classList.contains('open');
      if (wasOpen) {
        // close the same section
        ...
        // optional: bring the opened section into view on small screens
