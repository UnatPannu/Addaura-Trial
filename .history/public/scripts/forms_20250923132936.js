document.addEventListener('DOMContentLoaded', () => {
  ...
  const talentFileNameSpan = document.getElementById('resume-text'); // use span inside label

  // ----- SEND CV FORM -----
  const cvForm = document.getElementById('cv-form');
  const cvSubmitButton = document.getElementById('cv-submit');
  const cvFileInput = document.getElementById('cv-resume');
  const cvFileNameSpan = document.getElementById('cv-resume-text'); // span inside label
  ...
  if (talentForm && talentSubmitButton && talentFileInput && talentFileNameSpan) {
    ...
    talentFileInput.addEventListener('change', () => {
      talentFileNameSpan.textContent = talentFileInput.files.length > 0 ? talentFileInput.files[0].name : 'Upload File';
    });
    ...
    talentForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        talentForm.reset();
        talentFileNameSpan.textContent = 'Upload File';
      } catch (error) {
  ...
  if (cvForm && cvSubmitButton && cvFileInput && cvFileNameSpan) {
    cvFileInput.addEventListener('change', () => {
      cvFileNameSpan.textContent = cvFileInput.files.length > 0 ? cvFileInput.files[0].name : 'Upload File';
    });
    ...
    cvForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        await sendFormData(cvForm, 'Talent', cvAssetId);
        alert('CV submitted successfully!');
        cvForm.reset();
        cvFileNameSpan.textContent = 'Upload File';
      } catch (error) {
