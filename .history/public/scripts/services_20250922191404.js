...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (window.matchMedia("(max-width: 768px)").matches) {
    const industrySections = document.querySelectorAll('.industry-expertise .industry-section');

    // Close all dropdowns helper
    function closeAll() {
      industrySections.forEach(section => {
        section.classList.remove('expanded');
        const dropdown = section.querySelector('.mobile-dropdown');
        if (dropdown) {
    ...
    industrySections.forEach(section => {
      ...
      dropdown.style.maxHeight = "0";
      dropdown.style.overflow = "hidden";
      dropdown.style.transition = "max-height 0.4s cubic-bezier(.77,0,.18,1)";

      // Click handler on mobile header area only
      const clickableArea = section.querySelector('.mobile-header');
      if (!clickableArea) return;

      clickableArea.addEventListener('click', function(e) {
        // Prevent toggling when link or button inside clicked
