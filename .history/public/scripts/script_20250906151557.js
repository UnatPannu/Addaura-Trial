...
document.addEventListener("DOMContentLoaded", () => {
  ...
  firstSection.addEventListener("mouseenter", () => {
    navbar.classList.add("dark-mode");
    introSection.classList.add("dark-bg"); 
  });

  firstSection.addEventListener("mouseleave", () => {
    navbar.classList.remove("dark-mode");
    introSection.classList.remove("dark-bg");
  });
});

//something
document.addEventListener("DOMContentLoaded", () => {
  const thirdCard = document.querySelector(".intro-card-wrapper:nth-child(3) .intro-card");
  let hoverTimeout;

  thirdCard.addEventListener("mouseenter", () => {
    clearTimeout(hoverTimeout);
    thirdCard.classList.add("hovered");
  });

  thirdCard.addEventListener("mouseleave", () => {
    hoverTimeout = setTimeout(() => {
      thirdCard.classList.remove("hovered");
    }, 150); // 150ms delay before hover off
  });
});
