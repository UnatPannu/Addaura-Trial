...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const navbar = document.querySelector('.navbar');  // select by class only
  ...
  firstSection.addEventListener('click', () => {
    ...
    // Toggle navbar class based on expanded state 
    if (firstSection.classList.contains('expanded')) {
      navbar.classList.remove('light-navbar');
      // Add any class or style that makes navbar black background  
      navbar.classList.add('dark-navbar'); // example class for black background
    } else {
