...
export async function POST(request) {
  try {
    ...
    return new Response(JSON.stringify({ message: 'Form submission saved' }), { status: 200, headers: { "Access-Control-Allow-Origin": "*" } });
  } catch (error) {
    console.error("Form submission error:", error);
    return new Response(JSON.stringify({ error: 'Failed to save submission' }), { status: 500, headers: { "Access-Control-Allow-Origin": "*" } });
  }
}
