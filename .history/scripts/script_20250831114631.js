...
document.addEventListener("DOMContentLoaded", () => {
  ...
  rightPanel.addEventListener(
    ...
    (e) => {
      ...
      const atBottom =
        Math.ceil(rightPanel.scrollTop + rightPanel.clientHeight) >=
        rightPanel.scrollHeight;

      // if NOT at top scrolling up, and NOT at bottom scrolling down → consume scroll
      if (!(atTop && e.deltaY < 0) && !(atBottom && e.deltaY > 0)) {
        e.preventDefault();   // stop page from moving
        rightPanel.scrollTop += e.deltaY; // manually scroll inside panel
      }
    },
    { passive: false }
  );
